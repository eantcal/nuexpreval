...
template <typename Key, typename Symb> 
class symbol_map_t {
public:
    using key_t = Key;
    using symbol_t = Symb;
    ...
    const std::unordered_map<Key, Symb>& map() const noexcept { 
        return _symbols; 
    }
...
protected:
    std::unordered_map<Key, Symb>& map() noexcept { 
        return _symbols; 
    }
...
private:
    std::unordered_map<Key, Symb> _symbols;
};
